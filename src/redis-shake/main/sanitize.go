...
func SanitizeOptions(tp string) error {
	var err error
	if tp != conf.TypeDecode && tp != conf.TypeRestore && tp != conf.TypeJRestore && tp != conf.TypeDump && tp != conf.TypeSync && tp != conf.TypeJSync && tp != conf.TypeRump {
	...
	if tp == conf.TypeDump || tp == conf.TypeSync || tp == conf.TypeJSync {
	...
	} else if tp == conf.TypeRestore || tp == conf.TypeJRestore || tp == conf.TypeDecode {
	...
	OpLogFile := fmt.Sprintf("%s.oplog", conf.Options.Id)
	_ = os.Remove(OpLogFile)

	utils.OpLogRotater = &logRotate.Logger{
		Filename:   OpLogFile,
		MaxSize:    100, //MB
		MaxBackups: 50,
		MaxAge:     0,
	...
	log.OpLog = log.New(utils.OpLogRotater, "")
	...
	if tp == conf.TypeRestore || tp == conf.TypeJRestore || tp == conf.TypeSync || tp == conf.TypeRump {
