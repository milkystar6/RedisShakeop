...
type node struct {
	id     int    // id
	source string // source redis的连接方式
	output string // rdb file的名称 `local_dump.0`
}
		...
		go func(idx int) {
			...
			for {
				...
				case nd, ok := <-cmd.dumpChan:
					...
					//	output:         nd.output,
					//}
					dd := newdbDumper(nd.id, nd.source, conf.Options.SourcePasswordRaw, nd.output)
...
func newdbDumper(_id int, _source, _sourcePasswd, _output string) *dbDumper {
	return &dbDumper{
		id:             _id,
		source:         _source,
		sourcePassword: _sourcePasswd,
		output:         _output,
	}
}
