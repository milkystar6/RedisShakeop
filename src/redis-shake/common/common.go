...
func GetTotalLink() int {
	if conf.Options.Type == conf.TypeSync || conf.Options.Type == conf.TypeJSync || conf.Options.Type == conf.TypeRump || conf.Options.Type == conf.TypeDump {
		return len(conf.Options.SourceAddressList)
	} else if conf.Options.Type == conf.TypeDecode || conf.Options.Type == conf.TypeRestore || conf.Options.Type == conf.TypeJRestore {
