...
func GetRedisRole(target, authType, auth string, tlsEnable, tlsSkipVerify bool) (string, error) {
	...
	if err != nil {
		if err.Error() == CoidsErrMsg {
			...
			infoStr = CutRedisInfoSegment(infoStr, "replication")
		} else {
			return "", err
		}
	}
	...
	if value, ok := infoKV["role"]; ok {
		return value, nil
	} else {
		return "", fmt.Errorf("MissingRedisRoleInInfo")
	}
}
